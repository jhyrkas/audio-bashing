...
parser.add_argument('-whack_percent', dest='whack_percent', action='store', type=float, default=1., help='Percent (as 0.0-1.0) whacking to frequencies that qualify. When 0.0, the output is unchanged. When 1.0, energy is transfered such that some sinusoids are completely masked (in theory).')
...
for i in range(nfiles) :
    ...
    for j in range(len(notch_filts[i])) :
        ...
        #sf.write('tmp{i}_pre.wav'.format(i=tmp_index), processed_mask * (s_peak/np.max(np.abs(s_peak))), intended_fs)
